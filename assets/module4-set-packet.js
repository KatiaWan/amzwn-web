...
async function validate(bundle,asin,{allowSimulation=false}={}){
 ...
 check(request.model===p.record.model&&response.model===p.record.model&&response.id===p.record.responseId&&response.choices?.length===1&&response.choices[0].finish_reason==='stop'&&canonical(response.usage)===canonical(p.record.usage),'调用记录与响应不一致');
 ...
 check(canonical(analysis)===canonical(p.analysis)&&JSON.stringify(p.record.sampleIds)===JSON.stringify(analysis.sampleIds),'解析结果不一致');check(p.source==='SIMULATION'||!/SIMULATION|SYNTHETIC_TEST_ONLY/.test(p.rawResponse),'模拟结果不能作为真实结果');
